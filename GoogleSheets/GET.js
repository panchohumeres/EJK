...
var q_range_2='F15';
  var q_range= q_range_1.concat(':',q_range_2)
var range = sheet.getRange(q_range);
var svalues = range.getValues(); //values for "terms" query  
...
var fvalues = range.getValues(); //values for terms query
  ...
  for (var i = 0; i < svalues.length; i++){
    ...
    if (svalues[i][0].length>0 & JSON.stringify(svalues[i][1]).length>0){
    
      //if (typeof svalues[i][1]=== "string"){
      
      //  svalues[i][0]=svalues[i][0].concat('.keyword')
      
      //}
      
      var sort_dict={};
      sort_dict[svalues[i][0]]=svalues[i][1];
      
    sorts.push(sort_dict)
